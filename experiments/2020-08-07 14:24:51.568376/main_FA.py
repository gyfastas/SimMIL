...
from utils.utils import fix_bn, GaussianBlur, adjust_learning_rate, print_args, cal_metrics, cal_metrics_train,\
    _init_cluster_labels, momentum_update_key_encoder, batch_shuffle
...
# loss weight
parser.add_argument('--weight_self', type=float, default=0.3)
...
# feature extractor
if args.weight_self != 0 and config.loss_params.loss == 'MocoLossModule':
    encoder_q = ResBackbone(args.arch, args.pretrained)
    encoder_k = ResBackbone(args.arch, args.pretrained)
    encoder_q.cuda()
    encoder_k.cuda()
...
def train_multi_batch(epoch):
    ...
    for batch_idx, (idx, data, label, batch) in enumerate(tqdm(train_loader)):
        ...
        # print(idx)
        if args.cuda:
            bag_label, idx, batch= bag_label.cuda(non_blocking=True), idx.cuda(non_blocking=True), batch.cuda(non_blocking=True)
            if args.weight_self != 0 and config.loss_params.loss == 'MocoLossModule':
        ...
        if args.weight_self != 0 and config.loss_params.loss == 'MocoLossModule':
            ...
            with torch.no_grad():  # no gradient to keys
                momentum_update_key_encoder(encoder_q, encoder_k)  # update the key encoder
                # shuffle for making use of BN
                data[1], idx_unshuffle = batch_shuffle(data[1])
